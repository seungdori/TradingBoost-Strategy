...
async def log_telegram_event(
    user_id: str = None,
    okx_uid: str = None,  # okx_uid 파라미터 추가
    ...
    try:
        ...
        await redis_client.zadd(log_set_key, {log_data: log_score})

        # 2. okx_uid 기준 로그 저장 (새로운 방식)
        if okx_uid:
            okx_log_set_key = f"telegram:logs:by_okx_uid:{okx_uid}"
            await redis_client.zadd(okx_log_set_key, {log_data: log_score})
        ...
        await redis_client.sadd(index_key, log_id)

        # 4. Redis Pub/Sub 채널에 로그 발행 (두 채널 모두)
        # telegram_id 기준 채널 (기존)
        log_channel = LOG_CHANNEL_KEY.format(user_id=user_id)
        await redis_client.publish(log_channel, log_data)
        
        # okx_uid 기준 채널 (새로운)
        if okx_uid:
            okx_log_channel = f"telegram:log_channel:by_okx_uid:{okx_uid}"
            await redis_client.publish(okx_log_channel, log_data)

        # 5. 통계 업데이트
        if okx_uid:
            stats_key = f"telegram:stats:{okx_uid}"
            await redis_client.hincrby(stats_key, "total", 1)
            await redis_client.hincrby(stats_key, status, 1)
            await redis_client.hincrby(stats_key, f"category:{category}", 1)
...
async def enqueue_telegram_message(message_data):
    ...
    - Python: await redis_client.lrange(f"telegram:message_queue:{okx_uid}", 0, -1)
