...
class PositionStateManager:
    ...
    async def validate_position_state(self, user_id: str, symbol: str, side: str) -> bool:
        ...
        try:
            ...
            # Redis에 있는 포지션 정보
            redis_position_data = await redis_client.hgetall(position_key)
            ...
            await telegram_message.send_telegram_message(f"[{user_id}] [validate_position_state] error: {e}", debug=True)
    ...
    async def sync_position_state(self, user_id: str, symbol: str, exchange_position: Position | None):
        ...
        try:
            ...
            await telegram_message.send_telegram_message(f"[{user_id}] [sync_position_state] error: {e}", debug=True)
    ...
    async def cleanup_position_data(self, user_id: str, symbol: str, side: str):
        ...
        try:
            ...
            await telegram_message.send_telegram_message(f"[{user_id}] [cleanup_position_data] error: {e}", debug=True)
            logger.error(f"[cleanup_position_data] error: {e}")
            raise
    # ─────────────────────────────────────────────────────────────────────────
    ...
    async def update_position_state(
        ...
        try:
            result = await redis_client.transaction(update_operation, position_key)
    ...
    async def get_position_info(self, user_id: str, symbol: str, side: str) -> dict:
        ...
        try:
            position_key = self.get_position_key(user_id, symbol, side)
            return await redis_client.hgetall(position_key)
        except Exception as e:
            await telegram_message.send_telegram_message(f"[{user_id}] [get_position_info] error: {e}", debug=True)
