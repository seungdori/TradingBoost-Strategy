...
from HYPERRSI.src.api.routes.position import open_position_endpoint
    ...
    try:
        ...
        if settings['direction'] in ['롱숏', '롱']:
            ...
            if rsi_signals['is_oversold'] and trend_condition:
                try:
                    ...
                    try:
                        is_locked = await redis_client.exists(timeframe_lock_long_key)
                        locked_direction = "long"
                        if not is_locked:
                            is_locked = await redis_client.exists(timeframe_lock_short_key)
                            locked_direction = "short"
                    except Exception as e:
                        is_locked = False
                    if is_locked:
                        if locked_direction == "long":  
                            remaining_time = await redis_client.ttl(timeframe_lock_long_key)
                        else:
                            remaining_time = await redis_client.ttl(timeframe_lock_short_key)
                    ...
                    if expire_seconds > 0:
                        await redis_client.set(timeframe_lock_key, "locked", ex=expire_seconds)
        ...
        if settings['direction'] in ['롱숏', '숏']:
            ...
            try:
                ...
                timeframe_lock_key = timeframe_lock_long_key
            if is_locked:
                remaining_time = await redis_client.ttl(timeframe_lock_key)
                logger.info(f"[{user_id}] Position is locked for {symbol} with timeframe {timeframe_str}. Remaining time: {remaining_time}s")
                return  # 잠금이 있으면 포지션을 열지 않고 리턴
            ...
            if rsi_signals['is_overbought'] and trend_condition:
                try:
                    ...
                    await redis_client.set(f"user:{user_id}:position:{symbol}:short:dca_count", 1)
...
async def handle_existing_position(
    ...
    if initial_position_size is None:
        initial_position_size = await redis_client.get(f"user:{user_id}:position:{symbol}:{side}:initial_size")
        if initial_position_size is None:
            try:
                ...
                # Redis에 저장
                await redis_client.set(f"user:{user_id}:position:{symbol}:{side}:initial_size", initial_position_size)
                await redis_client.hset(f"user:{user_id}:position:{symbol}:{side}", "initial_size", initial_position_size)
                ...
                await redis_client.set(f"user:{user_id}:position:{symbol}:{side}:initial_size", initial_position_size)
                await redis_client.hset(f"user:{user_id}:position:{symbol}:{side}", "initial_size", initial_position_size)
    use_dual_side_settings = await redis_client.hget(f"user:{user_id}:dual_side", "use_dual_side_entry")
    trend_close_enabled = await redis_client.hget(f"user:{user_id}:dual_side", "dual_side_trend_close")
    ...
    is_locked = await redis_client.exists(timeframe_lock_key)
    if is_locked:
        remaining_time = await redis_client.ttl(timeframe_lock_key)
    ...
    if pyramiding_limit > 1:
        ...
        manual_calculated_initial_size = 0
        ...
        if check_dca_condition_result:
            ...
            if last_entry_size == 0 and dca_order_count > 1:
                ...
                try:
                    ...
                    if initial_size == 0:
                        initial_size_str = await redis_client.get(f"user:{user_id}:position:{symbol}:{side}:initial_size")
            ...
            try:
                ...
                manual_calculated_initial_size = await redis_client.get(f"user:{user_id}:position:{symbol}:{side}:initial_size")
                
                if dca_order_count == 1:
                    manual_calculated_initial_size = float(position_size)
                
                if dca_order_count > 1 and (manual_calculated_initial_size == 0 or manual_calculated_initial_size is None or manual_calculated_initial_size == "None" or manual_calculated_initial_size == "0"):
                    ...
                    if (trend_condition):
                        try:
                            ...
                            try:
                                ...
                                await redis_client.hset(f"user:{user_id}:position:{symbol}:long", "last_entry_size", new_position_entry_contract_size)
                            ...
                            dca_levels = await redis_client.lrange(dca_levels_key, 0, 0)  # 첫 번째 값만 가져오기
                            if dca_levels and len(dca_levels) > 0:
                                try:
                ...
                if (rsi_short_signals_condition and trend_condition) and dca_order_count + 1 <= settings.get('pyramiding_limit', 1):
                    ...
                    try:
                        ...
                        try:
                            ...
                            await redis_client.hset(f"user:{user_id}:position:{symbol}:short", "last_entry_size", new_position_entry_contract_size)
                        except Exception as e:
                ...
                try:
                    ...
                    if use_dual_side_settings == "true" and trend_close_enabled == "true":
                        await trading_service.close_position(
                            ...
                            symbol=symbol,
                            percent = 100,
                            comment="트랜드 반전으로 인한 양방향 포지션 종료",
                            side=dual_side
                        )
