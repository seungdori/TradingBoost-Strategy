...
async def check_valid_user(credentials: OkxCredentials):
    ...
    try:
        ...
        if (uid is not None) or uid != "":
            ...
            await redis_client.hset(f"user:{uid}:api:keys", mapping={"api_key": credentials.api_key, "api_secret": credentials.api_secret, "passphrase": credentials.passphrase, "uid": uid, "last_updated_time": last_updated_time, "last_update_time_kr": last_update_time_kr})
        ...
        cached_invitee_data = await redis_client.get(cache_key)
        print("================================================")
