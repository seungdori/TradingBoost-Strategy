...
async def update_contract_specifications(user_id: str):
    ...
    try:
        # 마지막 업데이트 시간 확인
        last_update = await redis_client.get("symbol_info:contract_specs_last_update")
        ...
        if not last_update or (current_time - int(last_update)) > 86400:
            async with get_exchange_context(user_id) as exchange:
                ...
                # Redis에 저장 (만료시간 없이)
                await redis_client.set("symbol_info:contract_specifications", json.dumps(specs_dict))
                await redis_client.set("symbol_info:contract_specs_last_update", str(current_time))
                
                return specs_dict
        
        # 기존 데이터 반환
        specs = await redis_client.get("symbol_info:contract_specifications")
...
async def get_contract_specifications(
    ...
    try:
        if force_update:
            ...
            await redis_client.delete("symbol_info:contract_specifications")
            await redis_client.delete("symbol_info:contract_specs_last_update")
...
async def get_history(
    ...
    try:
        async with get_exchange_context(str(user_id)) as exchange:
            history_list = await redis_client.lrange(keys['history'], 0, limit - 1)
            if not history_list:
                return []
