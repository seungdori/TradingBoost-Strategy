    ...
    try:
        ...
        if restart or is_running:
            ...
            if telegram_id and telegram_id != "":
                ...
                task_id = await redis_client.get(telegram_task_id_key)
            
            # okx_uid의 task_id 확인
            if not task_id:
                okx_task_id_key = f"user:{okx_uid}:task_id"
                task_id = await redis_client.get(okx_task_id_key)
            ...
            if task_id:
                ...
                try:
                    ...
                    if telegram_id:
                        await redis_client.delete(f"user:{telegram_id}:task_id")
                    
                    # okx_uid의 task_id 키 삭제
                    await redis_client.delete(f"user:{okx_uid}:task_id")
        ...
        try:
            task_running_exists = await redis_client.exists(task_running_key)
            if task_running_exists:
                logger.info(f"[{okx_uid}] 기존 태스크 실행 상태 초기화")
                await redis_client.delete(task_running_key)
        ...
        try:
            ...
            await redis_client.set(okx_status_key, "running")
            await redis_client.hset(
            ...
            if telegram_id:
                await redis_client.set(f"user:{telegram_id}:task_id", task.id)
            await redis_client.set(f"user:{okx_uid}:task_id", task.id)
            ...
            # Redis 상태 초기화
            if telegram_id:
                await redis_client.set(f"user:{telegram_id}:trading:status", "error")
            await redis_client.set(okx_status_key, "error")
        ...
        for key in status_keys:
            ...
            if status == "running":
                try:
                    ...
                    current_task_id = await redis_client.get(task_id_key)
                    ...
                    if current_task_id:
                        ...
                        await redis_client.set(key, "restarting")
                    ...
                    try:
                        ...
                        symbol = await redis_client.hget(preference_key, "symbol")
                        timeframe = await redis_client.hget(preference_key, "timeframe")
                        ...
                        await redis_client.set(key, "running") # 상태 키 사용
                        await redis_client.set(task_id_key, task.id) # 태스크 ID 키 사용
                        ...
                        # 상태를 'error'로 설정
                        await redis_client.set(key, "error") # 상태 키 사용
...
async def stop_trading(request: Request, user_id: Optional[str] = Query(None, description="사용자 ID (OKX UID 또는 텔레그램 ID)")):
    try:
        ...
        if telegram_id:
            ...
            if telegram_status == "running":
                await redis_client.set(telegram_status_key, "stopped")
        ...
        if okx_status == "running":
            await redis_client.set(okx_status_key, "stopped")
        ...
        # 종료 신호 설정
        if telegram_id:
            await redis_client.set(f"user:{telegram_id}:stop_signal", "true")
        await redis_client.set(f"user:{okx_uid}:stop_signal", "true")
...
async def get_all_running_users():
    ...
    try:
        ...
        status_keys = await redis_client.keys("user:*:trading:status") # 패턴 변경
        running_users = []
        ...
        for key in status_keys:
            ...
            if status == "running":
                ...
                try:
                    ...
                    try:
                        logger.info(f"사용자 {okx_uid}의 Redis 상태 초기화 중")
                        await redis_client.set(f"user:{okx_uid}:trading:status", "stopped") # 이미 위에서 설정함
                        ...
                        for key_to_del in keys_to_delete: # 변수명 변경 (key 중복 방지)
                            try:
                                await redis_client.delete(key_to_del)
                            except Exception as del_err:
            ...
            if status == "running":
                ...
                try:
                    ...
                    if current_task_id:
                        ...
                        celery_app.control.revoke(current_task_id, terminate=True)
                        await redis_client.delete(task_id_key)
                        await redis_client.set(key, "restarting") # 상태 키 사용
                        await asyncio.sleep(0.5)
...
async def get_user_trading_status(okx_uid: str): # user_id -> okx_uid
    ...
    try:
        ...
        trading_status = await redis_client.get(status_key)
        ...
        preferences = await redis_client.hgetall(preferences_key)
        if preferences:
            ...
            if "symbol" in preferences:
                ...
                position_state = await redis_client.get(position_state_key)
...
async def get_user_symbol_status(okx_uid: str, symbol: str): # user_id -> okx_uid
    ...
    try:
        ...
        symbol_status = await redis_client.get(symbol_status_key)
        ...
        long_position = await redis_client.get(long_position_key)
        if long_position:
            try:
        ...
        short_position = await redis_client.get(short_position_key)
        if short_position:
            try:
        ...
        long_dca = await redis_client.get(long_dca_key)
        short_dca = await redis_client.get(short_dca_key)
        ...
        symbol_settings = await redis_client.hgetall(symbol_settings_key)
        if symbol_settings:
