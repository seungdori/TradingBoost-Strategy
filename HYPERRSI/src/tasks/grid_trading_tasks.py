...
async def get_grid_trading_info(user_id: str) -> dict:
    """Grid 트레이딩 정보 가져오기"""
    key = REDIS_KEY_GRID_INFO.format(user_id=user_id)
    info_str = await redis_client.get(key)
    if not info_str:
        return {}
    try:
...
def run_grid_trading(self, exchange_name, enter_strategy, enter_symbol_count, 
    ...
    try:
        ...
        # 여기서 실제 grid_process.run_grid_trading 함수 호출 로직 구현
        # (실제 함수 구현은 기존 코드에서 마이그레이션 필요)
        from grid_process import run_grid_trading as execute_grid_trading
        
        result = execute_grid_trading(
...
def cancel_grid_trading_job(user_id: str):
    ...
    try:
        ...
        job_id = loop.run_until_complete(
            redis_client.get(REDIS_KEY_GRID_JOB_ID.format(user_id=user_id))
        )
