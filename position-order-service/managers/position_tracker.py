...
from shared.models.trading import Position, PositionStatus, PositionSide, Exchange
...
class PositionTracker:
    ...
    async def _handle_position_event(self, event: PositionEvent):
        ...
        Updates Redis state, calculates P&L, and persists to PostgreSQL.
    ...
    async def _persist_to_database(self, event: PositionEvent):
        """
        ...
        try:
            if event.event_type == EventType.POSITION_OPENED:
                ...
                # Calculate exit price and realized PnL from event
                exit_price = event.current_price or event.entry_price
                realized_pnl = event.unrealized_pnl or Decimal(0)

                await PositionRepository.update_close(
                    position_id=event.position_id,
                    exit_price=exit_price,
                    realized_pnl=realized_pnl,
                    fee=None  # TODO: Get fee from event if available
                )

        except Exception as e:
            # Log but don't fail - PostgreSQL is optional
            logger.warning(
                f"Failed to persist position to database: {e}",
                extra={"position_id": event.position_id}
            )
