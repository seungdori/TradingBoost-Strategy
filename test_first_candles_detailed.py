...
async def test_first_candles_detailed():
    """처음 캔들 상세 디버깅"""
    ...
    # 처음 100개 캔들 상세 출력 (더 많이 확인)
    print("\n" + "=" * 140)
    print("처음 100개 캔들의 모든 중간 계산 값")
    print("=" * 140)

    print(f"\n{'Idx':<5} {'Timestamp':<20} {'Close':>10} {'BB_St':>7} {'BB_MTF':>7} "
          f"{'CYC_B':>7} {'CYC_b':>7} {'CYC2_B':>8} {'CYC2_b':>8} {'trend':>6} {'Pine':>6} {'Match':<6}")
    print("-" * 140)
    ...
    for i in range(min(100, len(result))):
        ...
        cycle_bull_2nd = candle.get('CYCLE_Bull_2nd', False)
        cycle_bear_2nd = candle.get('CYCLE_Bear_2nd', False)
        ...
        cycle_bull_2nd_str = "Bull2" if cycle_bull_2nd else "-----"
        cycle_bear_2nd_str = "Bear2" if cycle_bear_2nd else "-----"

        match = "✅" if trend_state == pine_trend else "❌"

        print(f"{i:<5} {str(timestamp)[:19]:<20} {close:>10.2f} {bb_state:>7} {bb_state_mtf:>7} "
              f"{cycle_bull_str:>7} {cycle_bear_str:>7} {cycle_bull_2nd_str:>8} {cycle_bear_2nd_str:>8} "
              f"{trend_state:>6} {pine_trend:>6} {match:<6}")
    ...
    for i in range(min(10, len(candles_15m))):
        ...
        print(f"{i:<8} {str(ts)[:19]:<20} {close:>10.2f}")


if __name__ == "__main__":
    asyncio.run(test_first_candles_detailed())
